...
namespace TextAdventuresCS
{
    class Program
    {
        ...
        private static bool Teleport(Character you, string room, Place currentPlace)
        {
            bool moved = true;
            ...
            switch (room)
            {
                case "blue room":
                    ...
                    break;
                case "cupboard":
                    you.CurrentLocation = 2;
                    break;
                case "bedroom":
                    you.CurrentLocation = 3;
                    break;
                case "corridor":
                    you.CurrentLocation = 4;
                    break;
                case "guardroom":
                    you.CurrentLocation = 5;
                    break;
                case "gold cell":
                    you.CurrentLocation = 6;
                    break;
                case "silver cell":
                    you.CurrentLocation = 7;
                    break;
                case "cellar":
                    you.CurrentLocation = 8;
                    break;
                default:
                    moved = false;
                    break;
            ...
            if (CountInventory(items) < MaxInventory)
            {
                ...
                if (canGet)
                {
                    ...
                    if (items[indexOfItem].Status.Contains("gettable"))
                    {
                        ...
                        Console.WriteLine("You have retrieved that now.");
                    }
                }
            }
            else
        ...
        private static void LeaveItem(List<Item> items, string itemToLeave, int currentLocation)
        {
            int indexOfItem;
            indexOfItem = GetIndexOfItem(itemToLeave, -1, items);

            if (items[indexOfItem].Location == Inventory)
            {
                ChangeLocationOfItem(items, indexOfItem, currentLocation);
                Console.WriteLine("You have left that now.");
            }
            else if (indexOfItem == -1)
            {
                Console.WriteLine("You can't find {0}.", itemToLeave);
            }
            else if (!items[indexOfItem].Commands.Contains("leave"))
            {
                Console.WriteLine("You can't leave {0}.", itemToLeave);
            }
            else if (items[indexOfItem].Location >= MinimumIDForItem && items[GetIndexOfItem("", items[indexOfItem].Location, items)].Location != currentLocation)
            {
                Console.WriteLine("You can't find {0}.", itemToLeave);
        ...
        private static int CountInventory(List<Item> items)
        {
            int count = 0;
